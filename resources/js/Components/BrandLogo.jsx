export default function BrandLogo({
    ...
    const Mark = ({ size = 'h-8 w-8' }) => (
        <svg
            viewBox="0 0 316 316"
            xmlns="http://www.w3.org/2000/svg"
            aria-hidden="true"
            className={`${size} shrink-0 fill-current text-[var(--ink)]`}
        >
    ...
    const Wordmark = ({ compact = false }) => (
        <span className={`text-left leading-tight ${compact ? '' : ''}`}>
            <span className="block text-sm font-extrabold tracking-tight text-[var(--ink)] sm:text-base">
                Legions Group
            </span>
    ...
    if (variant === 'nav') {
        return (
            <span className={`inline-flex items-center gap-2.5 ${className}`}>
                <Mark size="h-7 w-7 sm:h-8 sm:w-8" />
    ...
    return (
        ...
        >
            <Mark size="h-14 w-14 sm:h-16 sm:w-16" />
